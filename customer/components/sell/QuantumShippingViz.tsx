...
function GlobeCameraControls({ animating }: { animating: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    if (animating && !interacting) {
      controls.target.lerp(US_FOCUS_POINT, delta * 0.45);
      controls.autoRotateSpeed = THREE.MathUtils.lerp(controls.autoRotateSpeed, 0.58, delta * 1.2);
    } else if (!interacting) {
      controls.target.lerp(home, delta * 0.55);
      controls.autoRotateSpeed = THREE.MathUtils.lerp(controls.autoRotateSpeed, 0.42, delta * 1.2);
    }
  });
...
function ShippingGlobeCanvas({
  ...
  return (
    ...
    >
      <GlobeCameraControls animating={animating} />
      <LiveGlobeScene animating={animating} optimizedIndex={optimizedIndex} />
    </Canvas>
  );
}
...
export default function QuantumShippingViz({
  ...
  if (!mounted) {
    return (
      <div className="h-[460px] w-full rounded-2xl bg-[#020308] border border-white/10 flex items-center justify-center">
        <div className="text-center">
  ...
  return (
    <div className="relative h-[460px] w-full rounded-2xl overflow-hidden border border-white/[0.08] bg-[#020308]">
      <ShippingGlobeCanvas animating={animating} optimizedIndex={optimizedIndex} />
